...
namespace FSFLauncherA3
{
    class ProgExterne
    {
        ...
        static public bool testTeamSpeakExistTaskForce3016()
        {
            try
            {
                if (File.Exists(FSFLauncherCore.cheminARMA3 + @"\userconfig\FSF-LauncherA3\TeamSpeak3TaskForce3016\TeamSpeak3\plugins\task_force_radio_win64.dll"))  //Si le fichier existe 
                {
                    return true;
                }
                else
